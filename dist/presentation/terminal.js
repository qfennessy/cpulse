 ...
 */
export function renderCardTerminal(card) {
    const lines = [];
    // Card label
 ...
 */
export function renderBriefingTerminal(cards, opening, closing, cardTransitions) {
    const lines = [];
    // Header
 ...
 */
export function formatMarkdownForTerminal(markdown) {
    const lines = [];
    lines.push('');
    lines.push(`${ANSI.bold}${ANSI.red}Commit Pulse${ANSI.reset}  ${ANSI.dim}Developer Briefing${ANSI.reset}`);
    lines.push('');
