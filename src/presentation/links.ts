 ...
 */
export function parseRepoString(repoString: string): GitHubRepo | null {
 ...
 */
export function prLink(repo: string, prNumber: number): string {
 ...
 */
export function prLinkWithTitle(repo: string, prNumber: number, title: string): string {
  const url = `https://github.com/${repo}/pull/${prNumber}`;
 ...
 */
export function commitLink(repo: string, sha: string): string {
 ...
 */
export function commitLinkWithMessage(repo: string, sha: string, message: string): string {
  const shortSha = sha.substring(0, 7);
  const url = `https://github.com/${repo}/commit/${sha}`;
 ...
 */
export function fileLink(repo: string, filePath: string, branch = 'main'): string {
 ...
 */
export function fileLinkWithLine(
  repo: string,
  filePath: string,
  lineNumber: number,
  branch = 'main'
 ...
 */
export function fileLinkWithRange(
  repo: string,
  filePath: string,
  startLine: number,
  endLine: number,
 ...
 */
export function branchCompareLink(repo: string, branch: string, baseBranch = 'main'): string {
 ...
 */
export function branchLink(repo: string, branch: string): string {
 ...
 */
export function issueLink(repo: string, issueNumber: number): string {
 ...
 */
export function linkifyGitHubReferences(text: string, defaultRepo?: string): string {
  let result = text;

  // Pattern: owner/repo#123 -> PR or issue link
  result = result.replace(
    /([a-zA-Z0-9_-]+\/[a-zA-Z0-9_.-]+)#(\d+)/g,
    (_, repo, num) => `[${repo}#${num}](https://github.com/${repo}/pull/${num})`
  );

  // Pattern: #123 (with default repo) -> PR or issue link
  if (defaultRepo) {
    result = result.replace(
      /(?<![a-zA-Z0-9_/])#(\d+)(?![a-zA-Z0-9])/g,
      (_, num) => `[#${num}](https://github.com/${defaultRepo}/pull/${num})`
    );
  }
  ...
  if (defaultRepo) {
    result = result.replace(
      /(?<![a-zA-Z0-9])([a-f0-9]{40})(?![a-zA-Z0-9])/g,
      (_, sha) => `[\`${sha.substring(0, 7)}\`](https://github.com/${defaultRepo}/commit/${sha})`
    );
  }
...
export function formatPRList(
  prs: Array<{ repo: string; number: number; title: string; isDraft?: boolean; reviewComments?: number }>
): string {
  return prs
    .map((pr) => {
...
export function formatCommitList(
  commits: Array<{ repo: string; sha: string; message: string; additions?: number; deletions?: number }>
): string {
  return commits
    .map((c) => {
 ...
 */
export function formatStaleBranchList(
  branches: Array<{ repo: string; branch: string; daysSinceUpdate?: number }>
): string {
  return branches
    .map((b) => {
