...
const BLOCKER_PATTERNS: Array<{ pattern: RegExp; type: 'blockedBy' | 'waitingOn' }> = [
 ...
 */
function extractSessionBlockers(session: ClaudeCodeSession): BlockerInfo[] {
  ...
  for (const message of session.messages) {
    ...
    for (const { pattern, type } of BLOCKER_PATTERNS) {
      ...
      if (match?.groups?.match) {
        ...
        if (matchedText.length < 3 || matchedText.length > 100) continue;
 ...
 */
export function extractBlockers(sessions: ClaudeCodeSession[]): BlockerInfo[] {
 ...
 */
export function groupBlockersBySource(
  blockers: BlockerInfo[]
): Map<string, BlockerInfo[]> {
  const grouped = new Map<string, BlockerInfo[]>();
 ...
 */
export function formatBlockersForBriefing(blockers: BlockerInfo[]): string {
