...
function generateQuestionId(session, index) {
    return `q-${session.id}-${index}`;
}
function extractContext(messages, index) {
...
function isQuestionResolved(messages, questionIndex) {
...
export function extractQuestions(session) {
    const questions = [];
    for (let i = 0; i < session.messages.length; i++) {
        ...
        const hasUnresolvedMarker = UNRESOLVED_PATTERNS.some((p) => p.test(message.content));
        ...
        if (!wasResolved || hasUnresolvedMarker) {
            ...
            if (questionText.length < 15)
                continue;
            if (/^(ok|okay|yes|no|sure|right)\?$/i.test(questionText))
                continue;
            questions.push({
...
export function extractAllQuestions(sessions) {
    const allQuestions = [];
...
export function groupQuestionsByProject(questions) {
...
export function formatQuestionsForBriefing(questions) {
