...
import type { ExtractedSignals, TodoItem, GitHubPR, PostMergeComment, BlockerInfo } from '../types/index.js';
...
function createMockTodo(content: string, overrides: Partial<TodoItem> = {}): TodoItem {
  return {
    content,
    status: 'pending',
    ...overrides,
...
function createMockPR(overrides: Partial<GitHubPR> = {}): GitHubPR {
  return {
    number: 1,
    title: 'Test PR',
    state: 'open',
...
function createMockComment(overrides: Partial<PostMergeComment> = {}): PostMergeComment {
  return {
    id: 1,
    prNumber: 1,
    prTitle: 'Test PR',
