...
export function saveBriefing(dataDir: string, briefing: Briefing): void {
...
export function loadBriefings(dataDir: string): Briefing[] {
  const filePath = getBriefingsPath(dataDir);
  ...
  for (const line of lines) {
    try {
      const briefing = JSON.parse(line) as Briefing;
...
export function getLatestBriefing(dataDir: string): Briefing | null {
...
export function getBriefingsByDateRange(
  dataDir: string,
  startDate: Date,
  endDate: Date
): Briefing[] {
...
export function getBriefingStats(dataDir: string): {
