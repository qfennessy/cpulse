...
import type { Config, ArticleCard } from '../types/index.js';
...
function baseTemplate(title: string, content: string, activeNav: string = ''): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <footer style="text-align: center; padding: 24px 20px; color: var(--text-muted); font-size: 14px; border-top: 1px solid var(--border);">
    Generated by <a href="https://github.com/qfennessy/cpulse" style="color: var(--primary); text-decoration: none;">cpulse</a>
  </footer>
</body>
</html>`;
}
...
export function renderBriefingPage(briefing: StoredBriefing): string {
...
export function renderAnalyticsPage(analytics: Analytics): string {
  ...
  const feedbackRate =
    analytics.feedbackStats.totalRated > 0
      ? Math.round(
          (analytics.feedbackStats.helpful / analytics.feedbackStats.totalRated) * 100
        )
      : 0;
