 ...
 * Created: 2026-01-12
 */
import { getAllBriefings } from '../storage/briefings.js';
export async function getAnalytics(dataDir) {
    ...
    for (const briefing of briefings) {
        for (const card of briefing.cards) {
            const projects = card.metadata?.projects;
            if (projects) {
    ...
    const averageCardsPerBriefing = briefings.length > 0 ? totalCards / briefings.length : 0;
    // Calculate streak
...
function calculateStreak(briefings) {
    if (briefings.length === 0) {
        return { current: 0, longest: 0 };
    }
    // Sort by date descending
    const sorted = [...briefings].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    // Get unique dates
    const dates = new Set();
    ...
    if (sortedDates[0] === today || sortedDates[0] === yesterday) {
        ...
        for (let i = 1; i < sortedDates.length; i++) {
            ...
            const dayDiff = (lastDate.getTime() - currentDate.getTime()) / (24 * 60 * 60 * 1000);
            if (dayDiff <= 1.5) {
                currentStreak++;
    ...
    for (let i = 1; i < allDates.length; i++) {
        ...
        const dayDiff = (curr.getTime() - prev.getTime()) / (24 * 60 * 60 * 1000);
        if (dayDiff <= 1.5) {
            tempStreak++;
        }
        else {
