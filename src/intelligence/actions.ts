 ...
 */
function getPRUrl(repo: string, number: number): string {
 ...
 */
export function extractActionItems(signals: ExtractedSignals): ActionItem[] {
 ...
 */
export function extractQuickWins(signals: ExtractedSignals): ActionItem[] {
  ...
  const smallTodos = signals.claudeCode.openTodos
    .filter(t => {
      ...
      // Skip complex tasks
      if (content.includes('refactor') || content.includes('rewrite')) return false;
      if (content.includes('implement') || content.includes('create')) return false;
 ...
 */
export function formatActionItemsForClosing(
