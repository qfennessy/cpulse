...
export async function createGitHubClient(config) {
    ...
    if (!token) {
        throw new Error('GitHub token not found. Set GITHUB_PERSONAL_ACCESS_TOKEN env var or add token to config.');
    }
        ...
        try {
            ...
            for (const branch of branches) {
                ...
                try {
                    ...
                    const lastCommitDate = new Date(branchDetail.commit.commit.author?.date || Date.now());
 ...
 */
async function getResolvedCommentUrls(octokit, owner, repo, prNumber) {
...
export async function getGitHubActivity(config, hoursBack = 168) {
    if (!config.enabled) {
